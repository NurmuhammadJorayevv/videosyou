    ...
    writeStream.on("finish", () => {
      ...
      bot
        .sendVideo(chatId, `${title}-${chatId}.mp4`, {
          caption: `*Video downloaded:* ${title} "by" @Video_downloader_Youtube1_bot 🏯
          `,
          thumb: thumbnailUrl,
...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendMessage(
    chatId,`Salom ${msg.from.first_name}

    Men Youtube dan video yuklovchi botman.
`

  )
  bot.sendMessage(chatId,`Ishlatilishi : Avval /yt deb yozib ,
   kn yonidan video Linki yuboriladi .
  Masalan "/yt  https://youtu.be/jnjnd" shu tartibda. 
`)
});
